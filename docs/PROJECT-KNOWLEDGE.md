...
- 3-panel UI: Scene List | Inspector | Preview
- Project state is separate from production state (no mutation)
- localStorage persistence with autosave (1s debounce)
- Studio handoff via `?project=<id>` URL parameter
- No Remotion hooks in Composer (preview is static)
- No TTS, no AI generation, no backend
- Template-specific inspectors for all 4 clean templates (scrapbook, cr7, cosmos, nodeflow)
- Scene operations: add, duplicate, delete, reorder, change kind, update content/duration/audio
- Audio foundation: optional per-scene audio path, non-breaking
- 47 regression tests in `composer.vitest.ts`
- Files: `src/composer/` (types, store, templates, scenes, validation, index, ComposerApp)
- Standalone: `preview/composer.html` + `preview/composer.jsx` (CDN-based, no build step)